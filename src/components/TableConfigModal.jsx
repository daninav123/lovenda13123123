    ...
    <Modal open={open} title={`Configurar Mesa ${table.id}`} onClose={onClose}>
      <div className="flex flex-col space-y-2">
        <label className="flex justify-between items-center">
          <span>Nombre:</span>
          <input value={name} onChange={e=>setName(e.target.value)} className="border rounded px-2 py-1 w-32" />
        </label>
        <label className="flex justify-between items-center">
          <span>Forma:</span>
          <select value={shape} onChange={e=>setShape(e.target.value)} className="border rounded px-2 py-1 w-32">
